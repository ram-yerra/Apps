    ...
    sync: function(callback,scope) {
        ...
        if (needsSync && me.fireEvent('beforesync', options) !== false) {
            me.proxy.batch(options, me.getBatchListeners());
				...
				//
				// JCM This code should run after the batch has completed,
				// JCM but operationcomplete and complete events make it
				// JCM kinda complicated to integrate it cleanly.
				//
				// JCM also load should only be called if some data did 
				// JCM get changed by the proxy.sync call.
				//
				// JCM and instead of just calling load, perhaps we could
				// JCM be passed back a list of the objects that changed
				// JCM and then only update those ones.
				//
				if (me.proxy.sync!==undefined) {
					me.proxy.sync(function(){
						me.load();
						if(callback){
							callback.call(scope)
						}
