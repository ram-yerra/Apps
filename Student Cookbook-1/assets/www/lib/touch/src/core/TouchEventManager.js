Ext.TouchEventManager = Ext.apply({}, {
    ...
    init : function() {
        ...
        this.useTouch = Ext.support.hasTouch && !Ext.support.isChrome;
    ...
    startTrack : function(touch, e) {
        ...
        if (track.events) {
            ...
            if (track.events.taphold || track.events.touchdown) {
                track.touchDownIntervalId = setInterval(Ext.createDelegate(me.touchDownHandler, track), me.touchDownInterval);
                track.tap = true;
            }
                        ...
                        for (id in targets) {
                            if (targets.hasOwnProperty(id)) {
                             for (x = 0, xln = targets[id].length; x < xln; x++) {
                                 ...
                                 if (options &&
                                     (options.isVertical === true && options.isHorizontal === false && absDeltaX >= absDeltaY) ||
                                     (options.isVertical === false && options.isHorizontal === true && absDeltaX <= absDeltaY)
                                 ) {
    ...
    touchDownHandler : function() {
        ...
        me.fireListeners('touchdown', track, {
            ...
            deltaX: track.lastEvent.deltaX,
            deltaY: track.lastEvent.deltaY,
            pageX: track.lastEvent.pageX,
            pageY: track.lastEvent.pageY,
            touch: track.touch
        });
