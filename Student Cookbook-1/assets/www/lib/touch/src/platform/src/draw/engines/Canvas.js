...
Ext.draw.Canvas = Ext.extend(Ext.draw.Surface, {
    fontSizeRE: /(?:^|\s)(\d+)px(?:\s|$)/,
    ...
    constructor: function(config) {
        ...
        Ext.draw.Canvas.superclass.constructor.call(this, config);
    },

    updateMaxBoundingBox: function(sprite) {
        if (sprite.type == "image" || sprite.type == "text") {
            delete sprite.realPath;
            Ext.draw.rectPath(this.getBBox(sprite));
            sprite.realPath = Ext.draw.rotateAndTranslatePath(sprite);
        }
    ...
    getBBox: function(sprite) {
        var bb,
            textAlign;
        if (sprite.realPath) {
            bb = Ext.draw.pathDimensions(sprite.realPath);
        } else {
            switch (sprite.type) {
                case "rect":
                case "image":
                ...
                break;
                case "path":
                    bb = Ext.draw.pathDimensions(sprite.path);
                break;
                case "text":
    ...
    applyChange: function(sprite) {
        ...
        switch (sprite.type) {
            case "circle":
                ...
                sprite.path = Ext.draw.path2curve(Ext.draw.ellipsePath(sprite));
            break;
            case "ellipse":
                sprite.path = Ext.draw.path2curve(Ext.draw.ellipsePath(sprite));
                break;
            case "path":
                sprite.path = Ext.draw.path2curve(sprite.path);
                break;
            case "rect":
                sprite.path = Ext.draw.path2curve(Ext.draw.rectPath(sprite));
                break;
            case "text":
                sprite.path = Ext.draw.path2curve(Ext.draw.rectPath(sprite));
                break;
    ...
    scrubAttrs: function(sprite) {
        var attrs = Ext.draw.Canvas.superclass.scrubAttrs.apply(this, [sprite]);
    ...
    change: function () {
        ...
        switch (this.type) {
            case "circle":
                ...
                this.path = Ext.draw.path2curve(Ext.draw.ellipsePath(this));
            break;
            case "ellipse":
                this.path = Ext.draw.path2curve(Ext.draw.ellipsePath(this));
                break;
            case "rect":
                this.path = Ext.draw.path2curve(Ext.draw.rectPath(this));
                break;
            case "text":
                this.path = Ext.draw.path2curve(Ext.draw.rectPath(this));
                break;
        ...
        if (this.path && this.path.length) {
            this.realPath = Ext.draw.rotateAndTranslatePath(this);
        }
